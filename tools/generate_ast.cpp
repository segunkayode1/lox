...
auto split(std::string statement, Container tokens)
    -> std::vector<std::string> {
  auto result = std::vector<std::string>{};
  auto start = statement.begin();
  auto const cmp = [&](auto const &c) { return tokens.contains(c); };
...
auto print_variant_declaration(std::string const &base_name,
                               std::ostream &filehpp, std::vector<line> lines)
    -> void {
  filehpp << "    using " << base_name << " = std::variant<" << base_name
          << "_Monostate\n";
...
auto print_class_constructor(std::ostream &os, line l) -> void {
  ...
  os << b_type << " t_" << b_name << ")\n";
  if (l.m_types.size() == 1) {
    os << header << ":m_" << b_name << "{std::move(t_" << b_name << ")} {}\n\n";
    return;
  }
  auto [f_type, f_name] = l.m_types[0];
  os << header << ":m_" << f_name << "{std::move(t_" << f_name << ")}\n";

  for (int i = 1; i < l.m_types.size() - 1; i++) {
    auto [type, name] = l.m_types[i];
    os << header << ",m_" << name << "{std::move(t_" << name << ")}\n";
  }
...
auto generate_files(std::string const &output_dir, std::string const &base_name,
                    std::vector<std::string> str_input) -> void {
  auto filehpp = std::ofstream{output_dir + "/include/" + lowercase(base_name) +
                               "/" + lowercase(base_name) + ".hpp"};
  std::vector<line> lines = generate(str_input);

  filehpp << "#ifndef LOX_" + uppercase(base_name) +
                 "_HPP\n"
                 "#define LOX_" +
                 uppercase(base_name) +
                 "_HPP\n"
  ...
  filehpp << "    struct " << base_name
          << "_Monostate : public std::monostate{\n"
             "        using std::monostate::monostate;\n"
             "    };\n\n";

  std::for_each(lines.begin(), lines.end(),
                [&](auto const &l) { print_class(filehpp, l); });

  filehpp << "};\n"
             "#endif";

  auto filecpp = std::ofstream{output_dir + "/src/" + lowercase(base_name) +
                               "/" + lowercase(base_name) + ".cpp"};
  filecpp << "#include <utility>\n\n";
  filecpp << "#include \"" + lowercase(base_name) + ".hpp\"\n\n";
  filecpp << "namespace lox{\n";

  std::for_each(lines.begin(), lines.end(),
                [&](auto const &l) { print_class_constructor(filecpp, l); });
...
auto main(int argc, char **argv) -> int {
  ...
  generate_files(output_dir, "Stmt",
                 std::vector<std::string>{
