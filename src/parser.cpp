...
namespace lox {
Parser::Parser(std::vector<Token> t_tokens) : m_tokens{std::move(t_tokens)} {};

auto Parser::match(auto... types) -> bool {
  auto const t_types = {types...};
  return std::ranges::any_of(t_types, [this](auto const &type) {
    if (check(type)) {
      advance();
      return true;
    }
...
auto Parser::expression() -> Expr { return assginment(); }
...
auto Parser::logical_or() -> Expr {
  ...
  while (match(Token_Type::OR)) {
    auto t_operator = previous();
    auto right = logical_and();
    expr = Box<Logical>{expr, t_operator, right};
  }
  return expr;
}
...
auto Parser::equality() -> Expr {
  ...
  while (match(BANG_EQUAL, EQUAL_EQUAL)) {
    ...
    expr = Box<Binary>{expr, t_operator, right};
  }
  return expr;
}
...
auto Parser::unary() -> Expr {
  ...
  if (match(MINUS, BANG)) {
    auto t_operator = previous();
    auto right = unary();
    return Box<Unary>{t_operator, right};
  }
...
auto Parser::call() -> Expr {
  ...
  if (match(Token_Type::LEFT_PAREN)) {
    ...
    while (match(Token_Type::COMMA)) {
      if (expressions.size() >= std::numeric_limits<unsigned char>::max()) {
        error(peek(), "Can't have more than 255 arguments");
      }
      expressions.push_back(expression());
    }

    auto token = consume(Token_Type::RIGHT_PAREN,
...
auto Parser::statement() -> Stmt {
  ...
  if (match(Token_Type::LEFT_BRACE)) {
    return Box<Block>{block_statement()};
  }
...
auto Parser::var_declaration() -> Stmt {
  auto const name = consume(Token_Type::IDENTIFIER, "Expected variable name.");
...
auto Parser::synchronize() -> void {
  ...
  using enum Token_Type;
  while (not is_at_end()) {
    if (previous().get_token_type() == SEMICOLON) {
      return;
    }
...
auto Parser::for_statement() -> Stmt {
  ...
  consume(SEMICOLON, "Expected ';' after lot_operator condition");
  ...
  if (not std::holds_alternative<Expr_Monostate>(increment)) {
    ...
    // clang-format off
    statements.push_back(Box<Expression>{increment}); // NOLINT : cant emplace as Stmt type is std::variant
    // clang-format on
    body = Box<Block>{statements};
  }
...
auto Parser::function(std::string const &kind) -> Stmt {
  ...
  if (not match(Token_Type::RIGHT_PAREN)) {
    auto first_param =
        consume(Token_Type::IDENTIFIER, "Expected paramater name");
    params.push_back(first_param);

    while (match(Token_Type::COMMA)) {
      if (params.size() >= std::numeric_limits<unsigned char>::max()) {
